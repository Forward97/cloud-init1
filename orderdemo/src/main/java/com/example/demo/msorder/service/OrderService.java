...
import com.example.demo.msorder.entity.OrderDetail;
...
public class OrderService {
     ...
     * @return
     */
    public Order queryOrderById0(String orderId) {
        ...
        for (OrderDetail orderDetail : orderDetails) {
            // 通过商品微服务查询商品详细数据
            Item item = itemService.queryItemById0(orderDetail.getItem()
                    .getId());
            if (null == item) {
                continue;
            }
    ...
    public Order queryOrderById(String orderId) {
        ...
        for (OrderDetail orderDetail : orderDetails) {
            // 通过商品微服务查询商品详细数据
            Item item = itemService.queryItemById(orderDetail.getItem()
                    .getId());
            if (null == item) {
                continue;
            }
